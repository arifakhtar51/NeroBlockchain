...
const Navbar = () => {
  ...
  const checkChain = async () => {
    if (window.ethereum) {
      ...
      if (chainId !== NERO_CHAIN_ID) {
        toast.error('Please switch to NERO Testnet to use the app!');
      }
    }
  };

  const switchToNero = async () => {
    ...
    try {
      ...
      if (switchError.code === 4902) {
        try {
          ...
          toast.error('Failed to add NERO Testnet');
        }
      } else {
  ...
  return (
    <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottom: '1px solid #eee', background: '#fff', color: '#222' }}>
      ...
      <div>
        {!chainOk && (
          <button onClick={switchToNero} style={{ marginRight: 12, background: '#ff9800', color: '#222' }}>Switch to NERO Testnet</button>
        )}
        {account ? (
          <>
