...
const Home = () => {
  ...
  const checkChain = async () => {
    if (window.ethereum) {
      ...
      setChainOk(chainId === NERO_CHAIN_ID);
    }
  };
  ...
  if (!chainOk) {
    return (
      <div style={{ padding: 24, color: '#ff9800', fontWeight: 'bold' }}>
        Please switch to the NERO Testnet to use the app.
      </div>
    );
  }
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24 }}>
        {videos.map((video) => (
          <div key={video.id} style={{ border: '1px solid #eee', borderRadius: 8, padding: 16, width: 240 }}>
            ...
            <div>Price: {formatEther(video.price)} Nero</div>
            <div>Valid View Time: {video.displayTime} sec</div>
            <button onClick={() => { setSelectedVideo(video); checkCanView(video); }} style={{ marginTop: 8 }}>Pay to View</button>
          </div>
        ))}
      </div>
      {selectedVideo && (
        <div style={{ marginTop: 32 }}>
          ...
          {canView ? (
            ...
            <button onClick={() => handlePayToView(selectedVideo)}>Pay to View</button>
          )}
        </div>
      )}
    </div>
